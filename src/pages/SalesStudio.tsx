...
import {
  ...
  type CardMix, type ComparisonInput, type HardwareSelection, type SubscriptionSelection,
...
const initial: ComparisonInput = {
  monthlyVolume: 5000, currentMonthly: 0, currentFixed: 15, currentVariablePercent: 1.5,
  currentTransactionCount: 0, currentPerTransaction: 0, mix: { ...emptyMix },
...
export function SalesStudio({ customerId, onOffer, photoInput, photoAvailable }: {
  ...
  const calculation = useMemo(() => {
    try { return { result: compareOffers(input), error: "" }; }
    catch (e) { return { result: null, error: (e as Error).message }; }
  }, [input]);
  const update = <K extends keyof ComparisonInput>(key: K, value: ComparisonInput[K]) =>
  ...
  const toggleHardware = (id: HardwareSelection["id"]) => {
    setInput(old => ({
      ...
      hardware: old.hardware.some(x => x.id === id)
        ...
        : [...old.hardware, { id, quantity: 1,
          price: hardwareCatalog.find(x => x.id === id)?.price ?? null }],
    }));
  };
  const toggleSubscription = (id: string) => setInput(old => ({
    ...old,
    subscriptions: old.subscriptions.some(s => s.id === id)
      ? old.subscriptions.filter(s => s.id !== id)
      : [...old.subscriptions, { id, monthly: subscriptions.find(x => x.id === id)?.monthly ?? null }],
  }));
  const result = calculation.result;
  const selected = result?.plans.find(p => p.id === (planId || result.best.id));
  const offerReady = Boolean(customerId && result && selected && !result.warning);
  return (
    <div className="sales-studio">
      <div className="section-intro">
        <div><h2>SumUp · Vertriebsstudio</h2>
          <p>Ist-Situation erfassen → Produkt wählen → Gebühren vergleichen → Angebot vorbereiten.</p></div>
        <External href={catalogSource}>SumUp Konditionen</External>
      </div>
      <p className="hint">Reguläre Referenzpreise für Deutschland · Stand {catalogCheckedAt}.
        Keine Aktionspreise oder individuellen Gebühren als reguläre Konditionen ausgewiesen.
        Hardware netto; Preis und Verfügbarkeit vor Versand des Angebots verifizieren.</p>
      <Card title="01 · Händler und Bedarf" eyebrow="QUALIFIZIERUNG">
        ...
        <div className="form-grid">
          {["POS", "Kiosk", "Bank Account", "Another Product"].map(x =>
            <label className="checkbox-field" key={x}><input type="checkbox"
              checked={productInterest.includes(x)}
              onChange={e => setProductInterest(old => e.target.checked ? [...old, x] : old.filter(y => y !== x))} />{x}</label>)}
        </div>
      ...
      <div className="analysis-grid">
        ...
        <Card title="03 · Kartenmix" eyebrow="TPV JE KARTENART">
          ...
          <label className="checkbox-field"><input type="checkbox"
          ...
          {result && result.missingMix > 0 && <p className="notice">Noch nicht aufgeteilt: {money(result.missingMix)} des TPV.</p>}
        </Card>
      </div>
      <Card title="04 · Hardware" eyebrow="REGULÄRE NETTOPREISE">
        <div className="solutions-grid">
          {hardwareCatalog.map(item => {
            ...
            return <div key={item.id} className="recommendation">
              <label className="checkbox-field"><input type="checkbox" checked={Boolean(selected)}
        ...
        <Field label={`Hardware-Rabatt: ${input.hardwareDiscount} % (0–25 %)`}>
          <input type="range" min="0" max="25" step="1" value={input.hardwareDiscount}
            onChange={e => update("hardwareDiscount", num(e.target.value))} />
        </Field>
        <External href={catalogHardwareSource}>Hardware-Preise nachprüfen</External>
        {result && <div className="mini-stats">
          <div><span>Rabattbetrag</span><b>{money(result.hardwareSaving)}</b></div>
          <div><span>Hardware einmalig netto</span><b>{money(result.hardwareNet)}</b></div>
        </div>}
      </Card>
      <Card title="05 · Lizenzen" eyebrow="WIEDERKEHRENDE NETTOKOSTEN, SOWEIT AUSGEWIESEN">
        <div className="solutions-grid">{subscriptions.map(s => {
          const chosen = input.subscriptions.find(x => x.id === s.id);
          return <div className="recommendation" key={s.id}>
            <label className="checkbox-field"><input type="checkbox" checked={Boolean(chosen)}
        ...
        <p className="hint">Geschäftskonto Plus ist öffentlich mit 25 € inkl. MwSt. ausgewiesen; vor einem Netto-Angebot steuerliche Behandlung gesondert prüfen. Jahres-, KDS- und Beauty-Preise nur nach Bestätigung eintragen.</p>
      </Card>
      <Card title="06 · Konditionen und Ergebnis" eyebrow="TRANSPARENTER 12-MONATS-VERGLEICH">
        {calculation.error && <p className="error" role="alert">{calculation.error}</p>}
        {result && <>
          {result.warning && <p className="notice">Vorläufige Modellrechnung: Kartenmix nicht vollständig und bestätigt. Vor einer verbindlichen Einsparungszusage aufschlüsseln.</p>}
          {result.customOffer && <p className="notice">Ab 10.000 € TPV monatlich kann ein individuelles SumUp-Angebot angefragt werden; nicht in den Standardtarifen berechnet.</p>}
          <div className="cost-row"><span>Bestand inkl. monatlicher Kosten</span><strong>{money(result.current)} / Monat</strong></div>
          <div className="cost-row"><span>Zusätzliche SumUp Lizenzen</span><strong>{money(result.fixedSubs)} / Monat</strong></div>
          {result.plans.map(plan => <div className="cost-row" key={plan.id}>
            <label className="checkbox-field"><input type="radio" name="sales-plan"
              checked={(planId || result.best.id) === plan.id}
              onChange={() => setPlanId(plan.id)} />{plan.title}
              {plan.id === result.best.id && <small> · niedrigste berechnete 12-Monats-Kosten</small>}
            </label>
            <strong>{money(plan.monthly)} / Monat<br /><small>Jahr 1 inkl. Hardware: {money(plan.year)}</small></strong>
          </div>)}
          {selected && <div className="recommendation">
            <h3>{selected.title}</h3>
            <div className="mini-stats">
              <div><span>Monatliche Differenz zum Bestand</span><b>{money(selected.savingsMonthly)}</b></div>
              <div><span>Jährliche Differenz inkl. Hardware</span><b>{money(selected.savingsYear)}</b></div>
              <div><span>Erster Monat inkl. Einmalkosten und Jahresabo ggf. vorab</span><b>{money(selected.firstMonth)}</b></div>
            </div>
            <p>Gebühren nach erfasstem Karteneinsatz, keine Zusage über künftige Umsatz- oder Kostenentwicklung. Jahresabo: 199 € vorab, für Vergleich auf zwölf Monate verteilt.</p>
          </div>}
          <div className="button-row">
            <button className="primary" disabled={!offerReady} onClick={() => {
              if (!selected) return;
              onOffer({
                division: "sumup", customer_id: customerId,
                lines: input.hardware.map(h => ({
                  name: hardwareCatalog.find(x => x.id === h.id)?.name || h.id,
                  quantity: h.quantity,
                  price: Math.round((h.price || 0) * (1 - input.hardwareDiscount / 100) * 100) / 100,
                  vat: 19,
                })),
                ...
                snapshot: { studio: input, comparison: result, plan: selected.id,
                  provider, start, productInterest, payout, checkedAt: catalogCheckedAt,
                  sources: [catalogSource, catalogHardwareSource] },
              });
            }}>Angebot vorbereiten</button>
          </div>
          {!customerId && <p className="hint">Zum Erstellen zuerst oben eine Kundenakte auswählen.</p>}
          {result.warning && <p className="hint">Für ein Angebot zuerst den Kartenmix vollständig aufteilen und bestätigen.</p>}
        </>}
      </Card>
    </div>
  );
