-- Shop product photos only. Images remain private; only the CRM owner may view or change them.
insert into storage.buckets (id,name,public,file_size_limit,allowed_mime_types)
values ('nx-vape-images','nx-vape-images',false,5000000,array['image/jpeg','image/png','image/webp'])
on conflict (id) do update
  set public=false,file_size_limit=5000000,allowed_mime_types=excluded.allowed_mime_types;
create policy nx_vape_images_owner_read on storage.objects
