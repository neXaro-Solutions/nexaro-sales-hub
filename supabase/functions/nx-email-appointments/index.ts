...
async function dispatch(){
 ...
 if(clock<"12:00"||clock>"12:29")return response({skipped:"Outside 12:00–12:29 Europe/Berlin"});
 ...
 if(!task||task.kind!=="Termin"||task.done||!task.customer_id||berlinDay(new Date(task.due_at))!==tomorrow||new Date(task.due_at).getTime()<=Date.now()){skipped++;continue}
 ...
 const appointmentDate=new Intl.DateTimeFormat("de-DE",{timeZone:"Europe/Berlin",weekday:"long",day:"2-digit",month:"long",year:"numeric"}).format(new Date(task.due_at));
